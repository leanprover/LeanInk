...
run_leanink () {
    ...
    if [[ -f "$directory/lakefile.lean" ]]; then
        ...
        (cd $directory && $LEANINK analyze $filename --x-enable-type-info --x-enable-docStrings --x-enable-semantic-token --lake lakefile.lean --prettify-output) || error "LeanInk failed - $file"
    else
        echo "Running LeanInk - $file"
        (cd $directory && $LEANINK analyze $filename --x-enable-type-info --x-enable-docStrings --x-enable-semantic-token --prettify-output) || error "LeanInk failed - $file"
    fi
}
